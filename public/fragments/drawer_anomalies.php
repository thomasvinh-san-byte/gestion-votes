...
$meeting = $repo->findMeetingForAnomalies($meetingId, $tenantId);
if (!$meeting) {
...
$openMotions = $repo->listOpenMotions($meetingId, $tenantId);

// Eligible voters: present/remote/proxy; fallback all active members
$eligibleRows = $repo->listEligibleVoters($meetingId, $tenantId);
$fallbackUsed = false;
if (!$eligibleRows) {
  $fallbackUsed = true;
  $eligibleRows = $repo->listAllActiveMembers($tenantId);
}
...
$motions = $repo->listMotionsForAnomalies($meetingId, $tenantId);
...
foreach ($motions as $mo) {
  ...
  $ballots = $repo->listBallotsForMotion($motionId);
...
// Proxies overview
try {
  $proxyRows = $repo->listProxySummary($meetingId);
