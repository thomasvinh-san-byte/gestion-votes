...
if ($method === 'GET') {
  ...
  $row = db_select_one("SELECT id, late_rule_quorum, late_rule_vote FROM meetings WHERE tenant_id=? AND id=?", [DEFAULT_TENANT_ID, $meetingId]);
...
api_guard_meeting_not_validated($meetingId);
  ...
  db_execute(
    ...
    [':q'=>$lrq, ':v'=>$lrv, ':t'=>DEFAULT_TENANT_ID, ':id'=>$meetingId]
  );
