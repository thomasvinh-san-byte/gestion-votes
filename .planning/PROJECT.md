...
**Shipped v9.0 Compliance & Robustness** (2026-04-03): Procuration PDF generation, RGPD compliance (data export, retention, erasure), transaction-level database locks for concurrent vote integrity, SSE cleanup and async error capture, list pagination with PV immutability and ARIA completeness. Plus UX interactivity overhaul: universal AgConfirm.ask() confirmations, per-page width strategy, AgToast bug fixes across all pages, SSE disconnect banner, wizard 2-column grid, unsaved changes warnings, animation timing contracts with prefers-reduced-motion. 6 phases, 4 plans (Phase 81), 125 commits.
...
- v10.0: Visual Identity Evolution — oklch color token foundation with warm-neutral stone palette (hue 75-95), dark mode warmed from navy to espresso (hue 78), single --radius-base (8px) for all corners, 3-level shadow vocabulary (sm/md/lg), alpha-based borders, 8 @property color registrations for CSS transitions, dashboard skeleton shimmer, zero hardcoded hex/rgba in any page CSS or Web Component, 21 critical-tokens inline blocks in oklch, focus rings tokenized across all 23 components.
