...
| **Quick run command** | `grep -c 'SEED_EVENTS\|showFallback' public/assets/js/pages/audit.js public/assets/js/pages/dashboard.js` |
...
- **After every task commit:** Run `grep -c 'SEED_EVENTS\|showFallback' public/assets/js/pages/audit.js public/assets/js/pages/dashboard.js` (expect all zeros)
...
| 17-02-01 | 02 | 1 | CLN-03 | static grep | `grep -c 'SEED_EVENTS' public/assets/js/pages/audit.js` | N/A | ⬜ pending |
...
- [ ] `tests/e2e/specs/audit.spec.js` — add smoke test: SEED_EVENTS strings absent from rendered output, error state renders on API failure
