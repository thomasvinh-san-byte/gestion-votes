...
AG-Vote est une application web de gestion de seances de vote formelles (assemblees generales, conseils d'administration, reunions d'organisation). Elle couvre le cycle complet : preparation, conduite en direct, vote electronique, calcul des resultats, validation et generation des livrables (PV, exports CSV).

### Quelles sont les technologies utilisees ?
- **Backend** : PHP 8.4+, sans framework
- **Base de donnees** : PostgreSQL 16+
- **Frontend** : HTML statique, JavaScript vanilla, Web Components
- **Temps reel** : SSE (Server-Sent Events) pour les mises a jour en direct
- **Serveur** : nginx + php-fpm + supervisord (Docker) ou serveur PHP integre pour le developpement
...
Voir le guide complet dans [dev/INSTALLATION.md](dev/INSTALLATION.md). En resume :
1. Installer PHP 8.4+ et PostgreSQL 16+
2. Creer la base de donnees et executer `database/schema-master.sql`
3. Configurer le fichier `.env` (copier depuis `.env.example`)
4. Lancer le serveur PHP ou configurer Docker
...
1. Via l'interface Admin (`/admin`) > onglet Utilisateurs > Nouveau
...
Ouvrir `/login.html` et saisir les identifiants de l'utilisateur (email et mot de passe). Cela cree une session PHP : les identifiants n'ont pas besoin d'etre ressaisis tant que la session est active.

### Quels sont les comptes de test par defaut ?
Les comptes crees par `database/seeds/02_test_users.sql` :
...
- **Roles systeme** (admin, operator, auditor, viewer) : permanents, definis dans la table `users`. Ils determinent l'acces aux pages et endpoints.
...
| Creer une seance | Oui | Oui | Non | Non | Non |
| Ouvrir/fermer un vote | Oui | Oui | Non | Non | Non |
| Valider la seance | Oui | Non | Non | Non | Oui |
| Consulter l'audit | Oui | Non | Oui | Non | Non |
| Voir les archives | Oui | Oui | Oui | Oui | Non |
| Exporter CSV/PV | Oui | Oui | Oui | Non | Non |
...
3. Le votant accede a `/vote`, choisit son vote et confirme
4. Le token est consomme (anti-rejeu) : il est impossible de voter deux fois
5. Le bulletin est enregistre
...
- **Par poids** : somme des poids des presents >= seuil
...
Par mot de passe (connexion via `/login.html`) ou par cle API. Chaque utilisateur possede un mot de passe hache (bcrypt) et optionnellement une cle API dont le hash SHA256 est stocke en base. Deux modes :
1. **Session PHP** : apres connexion via `/login.html`, une session est creee
2. **Header HTTP** : `X-Api-Key: ma-cle` (pour les appels API directs)

### Les mots de passe sont-ils stockes en clair ?
Non. Les mots de passe sont haches avec bcrypt. Les cles API sont hachees avec SHA256. Rien n'est stocke en clair cote serveur.
...
```
Puis acceder a `http://localhost:8080/login.html`.
...
- [dev/API.md](dev/API.md) : reference complete des endpoints
- [dev/SECURITY.md](dev/SECURITY.md) : securite et authentification

### Comment lancer les tests ?
```bash
./vendor/bin/phpunit tests/
```
Voir [dev/TESTS.md](dev/TESTS.md) pour la configuration complete.
...
Voir [dev/SECURITY.md](dev/SECURITY.md) pour la checklist complete. Points critiques :
...
4. Changer les mots de passe et cles API de test
5. Configurer HTTPS
